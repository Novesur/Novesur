...
<table width="100%" border="0">
  ...
  <tr>
    <td width="9%">&nbsp;</td>
    <td colspan="2" align="center"><strong>INFORME DE SERVICIO Nr° {{$servicioInforme->codigo}}</strong></td>
  </tr>
  <tr>
    <td><strong>FECHA : </strong></td>
    <td colspan="2">{{date('d-m-Y', strtotime($servicioInforme->fecha))}}</td>


  </tr>
  <tr>
    <td><strong>COD SERVICIO : </strong></td>
    <td colspan="2">{{$servicioInforme->servicio->codigo}}</td>
  </tr>

  <tr>
    <td><strong>CLIENTE : </strong></td>
    <td colspan="2">{{$servicioInforme->cliente}} </td>
  </tr>

  <tr>
    <td><strong>FECHA INICIO : </strong></td>
    <td colspan="2">{{date('d-m-Y', strtotime($servicioInforme->fechainicio))}} </td>
    <td><strong>FECHA FINAL : </strong></td>
    <td colspan="2">{{ date('d-m-Y', strtotime($servicioInforme->fechafinal))}} </td>
  </tr>

  <tr>
    <td><strong>DURACION : </strong></td>
    <td colspan="2">{{$servicioInforme->duracion}} </td>
  </tr>

{{--   <tr>
    <td><strong>O/S Nro : </strong></td>
    <td colspan="2">{{$servicioInforme->ord_servicio}} </td>
  </tr> --}}

  <tr>
    <td><strong>DETALLE DE SERVICIO : </strong></td>
    <td colspan="2">{{$servicioInforme->detservicio}} </td>
  </tr>
...
<br />
 <table width="100%" border="1">
  ...
  @if($servicioInforme_Materiales) @foreach ($servicioInforme_Materiales as $data)
  <tr>
...
<table width="100%" border="1">
  ...
  @if($servicioInforme_ManObra) @foreach ($servicioInforme_ManObra as $dataManObra)
  <tr>
  ...
  <tr>
    ...
    @php $sumaManObra = 0;  @endphp
        @if($servicioInforme_ManObra) @foreach ($servicioInforme_ManObra as $data)
        @php
...
<table width="100%" border="1">
  ...
  @if($servicioInforme_OtrosRequerimientos) @foreach ($servicioInforme_OtrosRequerimientos as $dataOtrosReq)
  <tr>
  ...
  <tr>
    ...
    @php $sumaOtrosReq = 0;  @endphp
        @if($servicioInforme_OtrosRequerimientos) @foreach ($servicioInforme_OtrosRequerimientos as $data)
        @php
