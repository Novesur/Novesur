...
<body>
    <table width="100%" border="0">
        ...
        <tr>
            <td align="center" style="font-size: 14px"><strong>PROYECTO - INFORME VALORIZACION Nr°
                    {{ $InformeValorizacion->codigo }}</strong></td>
        </tr>
    </table>
    <table width="100%" border="0">
        <tr>
        ...
        <tr>
            ...
            <td width="31%" style="font-size: 12px">{{ date('d-m-Y', strtotime($InformeValorizacion->fechainicio)) }}
            </td>
            <td width="23%" style="font-size: 12px"><strong>FECHA FINAL :</strong></td>
            <td width="30%" style="font-size: 12px"> {{ date('d-m-Y', strtotime($InformeValorizacion->fechafinal)) }}
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td><strong style="font-size: 14px">IMPORTE :</strong></td>
            <td colspan="3" style="font-size: 12px">{{ $InformeValorizacion->importe }}</td>
        </tr>
    ...
    <table width="100%" border="1">
        ...
        @if ($valorizacionReqMateriales)
            @foreach ($valorizacionReqMateriales as $data)
                <tr>
    ...
    <table width="100%" border="1">
        ...
        @if ($valorizacionManoObra)
            @foreach ($valorizacionManoObra as $dataManObra)
                <tr>
                    <td align="center" style="font-size: 12px">{{ $dataManObra->personal->nombres }}
                        {{ $dataManObra->personal->ApPaterno }} {{ $dataManObra->personal->ApMaterno }}</td>
    ...
    <table width="100%" border="1">
        <tr>
            ...
            <td width="51%" align="center" bgcolor="#33CCFF" style="font-size: 12px"><strong>UNID MEDIDA</strong>
            </td>
        </tr>
        @if ($valorizacionOtrosReq)
            @foreach ($valorizacionOtrosReq as $dataRque)
                <tr>
